...
function checkpoint(options) {
    ...
    this.stream = getCheckpointId(this.streamConstructor)
            ...
            return consumer.redisClient.set('checkpoint', newCheckpoint)
                .then(function() {
                    if (typeof consumer.checkpointHook === 'function')
                        return consumer.checkpointHook();
                });
        }
    }

    function retryStream() {
        consumer.stream = getCheckpointId.call(consumer, consumer.streamConstructor)
            .spread(createHighlandStream)
            .then(initiateStream);
    }
}
